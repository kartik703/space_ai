...
from app.ingestion import fetch_latest_kp  # ✅ live NOAA data

# ──────────────────────────────────────────
# Config
# ──────────────────────────────────────────
API_URL = os.getenv("API_URL")  # Railway FastAPI endpoint if available
...
# ──────────────────────────────────────────
# Latest Kp (live NOAA fetch with cache)
# ──────────────────────────────────────────
@st.cache_data(ttl=600)  # cache for 10 minutes
def get_latest_kp():
    try:
        return fetch_latest_kp()
    except Exception as e:
        st.error(f"⚠️ Could not fetch live Kp index. Using fallback (2.0). Error: {e}")
        return 2.0

latest_kp = get_latest_kp()
...
with c3:
    ...
    st.success(msgs["Energy"]) if "⚠️" not in msgs["Energy"] else st.warning(msgs["Energy"])

# ──────────────────────────────────────────
# Forecasts (API first; fallback local)
# ──────────────────────────────────────────
...
try:
    if API_URL:
        ...
        st.caption(f"🔵 Using API at {API_URL}/forecast")
    else:
    ...
    try:
        ...
        st.error(f"❌ Could not generate forecasts. Error: {e}")
...
if results:
    ...
    hist_line = base.transform_filter(alt.datum.Type == "History").mark_line(color="#60a5fa").encode(y=alt.Y("Kp:Q", title="Kp"))
    band = alt.Chart(fdf).mark_area(opacity=0.2, color="#f59e0b").encode(x="Time:T", y="Kp_lo:Q", y2="Kp_hi:Q")
